...
files = [f'end_effector_data_Estirar_NoC_{i}.csv' for i in range(3, 15)]

# Generar colores en degradé
num_files = len(files)
colors = cm.plasma(np.linspace(0, 1, num_files))
...
for idx, file in enumerate(files):
    i = idx + 3  # Número del archivo (3 a 14)
    ...
    plt.plot(Pressure_PSI, y1_ad, color=color, marker=',', label=f'Cantidad de Anillos = {i}')
...
plt.savefig('Combined_Displacement_Pressure.png', dpi=300)
plt.show()
